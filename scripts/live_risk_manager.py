...
class LiveRiskManager:
    ...
    def can_trade(self, symbol: str, timestamp: float = None) -> bool:
        """Return True if `symbol` is allowed to trade right now.

        Enforces the per-symbol anti-overtrade cooldown: returns False while the
        time since the last recorded exit is below `trade_cooldown_seconds`.
        """
        now = timestamp if timestamp is not None else time.time()
        last_exit = self._last_exit_ts.get(symbol)
        if last_exit is not None and (now - last_exit) < self.trade_cooldown_seconds:
            remaining = self.trade_cooldown_seconds - (now - last_exit)
            logger.info(
                f"Trade veto for {symbol}: cooldown active, {remaining:.0f}s remaining"
            )
            try:
