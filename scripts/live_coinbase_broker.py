...
class LiveCoinbaseBroker:
    ...
    def _make_request(self, method, endpoint, data=None):
        ...
        try:
            # Use a single dispatch so every verb (incl. DELETE for cancel_order)
            # is handled — the old GET/POST-only branch left `response` unbound
            # on DELETE, raising UnboundLocalError and silently failing cancels.
            response = requests.request(
                method, url, headers=headers,
                data=body if body else None, timeout=10
            )
            response.raise_for_status()
            # Some successful responses (e.g. DELETE) may carry no body.
            if not response.content:
                return {}
            return response.json()
        except requests.exceptions.HTTPError as e:
            body_text = getattr(e.response, 'text', '')
            print(f"Coinbase API HTTP error ({method} {path}): {e} | {body_text}")
            return None
        except Exception as e:
            print(f"Coinbase API error ({method} {path}): {e}")
            return None
    ...
    def place_market_order(self, symbol, side, size):
        ...
        order_data = {
            ...
            # Idempotency: a retried POST with the same client_oid won't double-fill.
    ...
    def _format_size(size) -> str:
        """Quantize an order size to 8 dp (ROUND_DOWN) to avoid binary-float
        artifacts (e.g. 0.1 + 0.2) that Coinbase rejects or rounds unpredictably."""
        return format(Decimal(str(size)).quantize(Decimal('0.00000001'), rounding=ROUND_DOWN), 'f')
