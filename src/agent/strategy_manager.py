...
class StrategyManager:
    ...
    def generate_signals(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Expose the raw per-strategy view alongside the blend so the
            # execution layer can run strategies as independent books
            # (strategy_execution_mode: 'parallel').
            ensemble_signals['per_strategy'] = strategy_signals
    ...
    def _adaptive_confidence_ensemble(self, strategy_signals: Dict[str, Dict[str, Any]], 
                                     symbol: str, data: Dict[str, Any] = None) -> Dict[str, Any]:
        """
        Dynamically adjusts weights based on recent volatility/performance.
        Conservative strategies get more weight during high volatility.
        """
        action_scores = {'buy': 0.0, 'sell': 0.0, 'hold': 0.0}
        total_weight = 0.0
        total_confidence = 0.0
        ...
        for name, signals in strategy_signals.items():
            ...
            action_scores[action] += base_weight * confidence
            total_weight += base_weight
            total_confidence += base_weight * confidence
            total_position_size += base_weight * position_size
