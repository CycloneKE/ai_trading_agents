...
class OrderRequest:
    ...
    # Deterministic per-logical-order id for broker-side idempotency / dedupe.
...
class OrderExecutionEngine:
        ...
        try:
            ...
            for i in range(slices):
                slice_order = OrderRequest(
                    ...
                    client_order_id=f"{parent_order_id}_slice_{i+1}",
                )
    ...
    def _schedule_order(self, order: OrderRequest, execution_time: datetime):
        ...
        # Use a globally-unique id: len(pending_orders) collides after deletions
        # and would overwrite an existing scheduled slice, silently dropping it.
        scheduled_id = f"scheduled_{uuid.uuid4().hex}"
        if order.client_order_id is None:
            order.client_order_id = scheduled_id
    ...
    def update_orders(self):
        ...
        try:
            ...
            # Snapshot: _continue_iceberg adds new keys to pending_orders mid-loop,
            # which would raise "dict changed size during iteration".
            for order_id, order_info in list(self.pending_orders.items()):
                # Handle scheduled orders
                if order_info.get('status') == 'scheduled':
                    if current_time >= order_info['execution_time']:
                        # Mark as submitting BEFORE placing the order. If the broker
                        # accepts it but the response is lost, we must NOT resubmit on
                        # the next tick — a duplicate fill is worse than a missed retry.
                        order_info['status'] = 'submitting'
                        response = self.broker_manager.place_order(order_info['order'])
                        if response:
                            order_info['response'] = response
                            order_info['status'] = 'submitted'
                        else:
                            order_info['status'] = 'submit_failed'
                            logger.error(
                                f"Scheduled order for {order_info['order'].symbol} returned "
                                f"no response; not retrying to avoid duplicate fills "
                                f"(client_order_id={order_info['order'].client_order_id})."
                            )
                    continue
