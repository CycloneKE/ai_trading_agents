    ...
    def seed_history(self, history: Dict[str, Any]):
        ...
        try:
            ...
            for t, e in zip(timestamps, equities):
                ts = datetime.fromisoformat(t) if isinstance(t, str) else datetime.fromtimestamp(t)
                self.record_portfolio_value(float(e), ts)
                
            logger.info(f"Successfully seeded {len(timestamps)} history points")
        except Exception as e:
            logger.error(f"Error seeding performance history: {e}")
