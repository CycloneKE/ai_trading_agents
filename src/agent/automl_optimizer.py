...
# --- Safe model (de)serialization -------------------------------------------
# pickle.load() executes arbitrary code embedded in the byte stream, so loading
# a tampered or attacker-supplied model file is a remote-code-execution risk.
# Defense is two-layered:
#   1. HMAC integrity — model files are signed on save with MODEL_SIGNING_KEY
#      and the signature is verified before any deserialization, so a file we
#      did not sign (or that was modified) is refused outright.
#   2. RestrictedUnpickler — even for trusted files, only an allowlist of
#      data/model modules may be reconstructed, blocking os/subprocess/eval
#      style gadget chains as defense in depth.
_MODEL_SIGNING_KEY = os.getenv('MODEL_SIGNING_KEY', '').encode()

_ALLOWED_PICKLE_ROOTS = frozenset({
    'numpy', 'pandas', 'sklearn', 'scipy',
    'builtins', '__builtin__', 'collections', 'copyreg',
})
...
class _RestrictedUnpickler(pickle.Unpickler):
    """Unpickler that only reconstructs objects from an allowlist of modules."""

    def find_class(self, module, name):
        root = module.split('.', 1)[0]
        if root in _ALLOWED_PICKLE_ROOTS and not (
            root in ('builtins', '__builtin__') and name in _BLOCKED_BUILTINS
        ):
            return super().find_class(module, name)
        raise pickle.UnpicklingError(
            f"Refusing to unpickle disallowed object {module}.{name}"
        )


def _model_signature(data: bytes) -> str:
    """Return the HMAC-SHA256 signature of model bytes using the signing key."""
    return hmac.new(_MODEL_SIGNING_KEY, data, hashlib.sha256).hexdigest()
    ...
    def save_model(self, model_name: str, filepath: str):
        ...
        try:
            if model_name in self.best_models:
                data = pickle.dumps(self.best_models[model_name])
                with open(filepath, 'wb') as f:
                    f.write(data)
                if _MODEL_SIGNING_KEY:
                    with open(filepath + '.sig', 'w') as sf:
                        sf.write(_model_signature(data))
                else:
                    logger.warning(
                        "MODEL_SIGNING_KEY not set — model %s saved without an "
                        "integrity signature; load-time verification will be skipped.",
                        filepath,
                    )
        ...
        try:
            ...
            if _MODEL_SIGNING_KEY:
                ...
                try:
                    ...
                    logger.error(
                        "Refusing to load %s: missing signature file %s",
                        filepath, sig_path,
                    )
                    return {}
                if not hmac.compare_digest(expected, _model_signature(data)):
                    logger.error(
                        "Refusing to load %s: signature mismatch (file tampered "
                        "or not produced by this system)", filepath,
                    )
                    return {}
            else:
                logger.warning(
                    "MODEL_SIGNING_KEY not set — loading %s without integrity "
                    "verification; restricted unpickler still applies.", filepath,
                )

            model_data = _RestrictedUnpickler(io.BytesIO(data)).load()
