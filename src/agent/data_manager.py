...
# Import each connector independently: one missing/broken module must not
# disable the others (a shared try block silently killed Finnhub market+news
# when the coinbase/oanda connectors went away).
try:
    from src.connectors.market_data_connector import FinnhubConnector
except ImportError:
    FinnhubConnector = None
try:
    from src.connectors.news_connector import FinnhubNewsConnector, RedditConnector
except ImportError:
...
try:
    from src.connectors.oanda_connector import OandaConnector
except ImportError:
    OandaConnector = None

CONNECTORS_AVAILABLE = any(
    c is not None
    for c in (FinnhubConnector, FinnhubNewsConnector, CoinbaseConnector, OandaConnector)
)
...
try:
    from src.connectors.mocked_connector import MockedConnector
    MOCK_AVAILABLE = True
except Exception:
...
class DataManager:
    ...
    def _init_connectors(self):
        ...
        # Initialize NSE Kenya connector
        nse_cfg = self.config.get('connectors', {}).get('nse', {})
        if NSE_AVAILABLE and NSEConnector is not None and nse_cfg.get('enabled', False):
            try:
                self.connectors['nse'] = NSEConnector(nse_cfg, database_manager=self.config.get('_database_manager'))
                logger.info("Initialized NSE Kenya data connector")
            except Exception as e:
                logger.warning(f"Failed to initialize NSE connector: {e}")
    ...
    def _collect_market_data(self, symbols: List[str]):
        """
        Collect market data, trying connectors in priority order.

        Sources are alternatives for the SAME quotes (real_data already
        wraps Finnhub + Alpha Vantage), so the first one that delivers wins —
        querying the rest would just duplicate vendor API calls.
        """
        ...
        for connector_name in market_connectors:
            ...
            if connector_name != 'real_data' and not getattr(connector, 'is_connected', False):
                continue
            try:
                ...
                if data:
                    ...
                    data_collected = True
                    break  # primary delivered; skip redundant fallback fetches
            except Exception as e:
    ...
    def _collect_news_data(self, symbols: List[str]):
        ...
        for connector_name in news_connectors:
            if connector_name in self.connectors:
                connector = self.connectors[connector_name]
                # Guard with getattr: a connector lacking `is_connected` would
                # otherwise raise AttributeError and silently kill news collection.
